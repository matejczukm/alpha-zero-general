import sys
sys.path.append('..')
from utils import *

import argparse
from keras.models import Model
from keras.layers import Input, Reshape, Conv2D, BatchNormalization, Activation, Flatten, Dense, Dropout
from keras.optimizers import Adam

class HexNNet():
    def __init__(self, game, args):
        # Game parameters
        self.board_x, self.board_y = game.getBoardSize()
        self.action_size = game.getActionSize()
        self.args = args

        # Neural Network
        self.input_boards = Input(shape=(self.board_x, self.board_y))  # Input: batch_size x board_x x board_y

        # Reshape input to add a channel dimension
        x_image = Reshape((self.board_x, self.board_y, 1))(self.input_boards)  # batch_size x board_x x board_y x 1

        # Convolutional layers with strided convolutions
        h_conv1 = Activation('relu')(
            BatchNormalization(axis=3)(
                Conv2D(args.num_channels, 3, padding='same', use_bias=False)(x_image)
            )
        )  # batch_size x board_x x board_y x num_channels

        h_conv2 = Activation('relu')(
            BatchNormalization(axis=3)(
                Conv2D(args.num_channels, 3, strides=2, padding='same', use_bias=False)(h_conv1)
            )
        )  # batch_size x (board_x/2) x (board_y/2) x num_channels

        h_conv3 = Activation('relu')(
            BatchNormalization(axis=3)(
                Conv2D(args.num_channels * 2, 3, strides=2, padding='same', use_bias=False)(h_conv2)
            )
        )  # batch_size x (board_x/4) x (board_y/4) x (2 * num_channels)

        h_conv4 = Activation('relu')(
            BatchNormalization(axis=3)(
                Conv2D(args.num_channels * 4, 3, strides=2, padding='same', use_bias=False)(h_conv3)
            )
        )  # batch_size x (board_x/8) x (board_y/8) x (4 * num_channels)

        # Flatten the output
        h_conv4_flat = Flatten()(h_conv4)  # batch_size x (board_x/8 * board_y/8 * 4 * num_channels)

        # Fully connected layers
        s_fc1 = Dropout(args.dropout)(
            Activation('relu')(
                BatchNormalization(axis=1)(
                    Dense(1024, use_bias=False)(h_conv4_flat)
                )
            )
        )  # batch_size x 1024

        s_fc2 = Dropout(args.dropout)(
            Activation('relu')(
                BatchNormalization(axis=1)(
                    Dense(512, use_bias=False)(s_fc1)
                )
            )
        )  # batch_size x 512

        # Policy head (action probabilities)
        self.pi = Dense(self.action_size, activation='softmax', name='pi')(s_fc2)  # batch_size x self.action_size

        # Value head (expected outcome)
        self.v = Dense(1, activation='tanh', name='v')(s_fc2)  # batch_size x 1

        # Model definition
        self.model = Model(inputs=self.input_boards, outputs=[self.pi, self.v])
        self.model.compile(loss=['categorical_crossentropy', 'mean_squared_error'], optimizer=Adam(args.lr))
