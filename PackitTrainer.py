...
class PackitTrainer():
    ...
    def loadModel(self, local = False, local_folder=None, local_filename = None, hf_repo_id='lgfn/packit-polygons-models', hf_filename = None):
    ...
    def train(self,
        ...
        if checkpoint_path == '':
            if self.mode == 'triangular':
                checkpoint_path = './packit-polygons-models/' + self.model_framework + '/triangle_models/size_'+str(self.size)
            else:
                checkpoint_path = './packit-polygons-models/' + self.model_framework + '/hex_models/size_'+str(self.size)
        if best_filename == '':
    ...
    def getConfig(self):
        return {
            'size' : self.size,
            'mode' : self.mode,
            'model_framwork' : self.model_framework
        }
